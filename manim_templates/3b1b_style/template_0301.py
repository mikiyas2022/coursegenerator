"""
Template 0301 — COMPARE: Theory vs Practice TEAL_ACCENT
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            ...
            lt = latin_text("Theory", font_size=FONT_SIZE_LABEL, color=TEAL_ACCENT)
            lt.move_to(LEFT*3)
            clamp_to_screen(lt)
            rt = latin_text("Practice", font_size=FONT_SIZE_LABEL, color=VECTOR_COLOR)
            rt.move_to(RIGHT*3)
            clamp_to_screen(rt)
