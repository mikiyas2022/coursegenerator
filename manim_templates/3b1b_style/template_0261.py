"""
Template 0261 — VECTOR: angle_t.get_ ROSE_COLOR
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            ...
            vec = always_redraw(lambda: Arrow(axes.c2p(0,0), axes.c2p(2*np.cos(angle_t.get_value()*0.5), 2*np.sin(angle_t.get_value()*0.5)), buff=0, color=ROSE_COLOR, stroke_width=5, max_tip_length_to_length_ratio=0.15))
            self.add(vec)
