"""
Template 179 — NUMBER LINE: [0,100] FORCE_COLOR
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            nl = NumberLine(x_range=[0,100,1], length=8, color=AXIS_COLOR,
                           include_numbers=True, font_size=FONT_SIZE_CAPTION)
            nl.move_to(DOWN*0.5)
            t = ValueTracker(0)
            marker = always_redraw(lambda: Triangle(fill_color=FORCE_COLOR, fill_opacity=1,
            ...
            val_lbl = always_redraw(lambda: latin_text(f"{t.get_value():.1f}",
                font_size=FONT_SIZE_CAPTION, color=FORCE_COLOR).next_to(nl.n2p(t.get_value()), UP, buff=0.35))
            self.play(Create(nl), run_time=tracker.duration*0.2)
            self.add(marker, val_lbl)
            self.play(t.animate.set_value(100), run_time=tracker.duration*0.6, rate_func=smooth)
            self.wait(tracker.duration*0.1)

        self.clear()
        with self.voiceover(text="<NARRATION_PLACEHOLDER_1>") as tracker:
            obj = formula_box("<NARRATION_PLACEHOLDER_1>", color=FORCE_COLOR)
            obj.move_to(ORIGIN)
