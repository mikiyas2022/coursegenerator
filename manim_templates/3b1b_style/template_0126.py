"""
Template 0126 — CURVE: beat pattern ERROR_COLOR
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            ...
            import math
            curve = axes.plot(lambda x: math.sin(x)*math.cos(x*0.5), x_range=[-3.5,3.5], color=ERROR_COLOR, stroke_width=3)
            lbl = latin_text("beat pattern", font_size=FONT_SIZE_CAPTION, color=ERROR_COLOR)
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_1>") as tracker:
            fb = formula_box("<NARRATION_PLACEHOLDER_0>", color=ERROR_COLOR)
            fb.move_to(ORIGIN)
