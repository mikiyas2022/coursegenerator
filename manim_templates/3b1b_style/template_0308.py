"""
Template 0308 — COMPARE: Question vs Answer VECTOR_COLOR
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            ...
            lt = latin_text("Question", font_size=FONT_SIZE_LABEL, color=VECTOR_COLOR)
            lt.move_to(LEFT*3)
            clamp_to_screen(lt)
            rt = latin_text("Answer", font_size=FONT_SIZE_LABEL, color=ROSE_COLOR)
            rt.move_to(RIGHT*3)
            clamp_to_screen(rt)
