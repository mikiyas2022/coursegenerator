"""
Template 0397 — VENN: STAR_YELLOW VECTOR_COLOR
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            left_c = Circle(radius=1.5, color=STAR_YELLOW, fill_opacity=0.1, stroke_width=3).shift(LEFT*1)
            right_c = Circle(radius=1.5, color=VECTOR_COLOR, fill_opacity=0.1, stroke_width=3).shift(RIGHT*1)
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_1>") as tracker:
            ...
            self.play(Circumscribe(fb, color=VECTOR_COLOR, time_width=2), run_time=tracker.duration*0.35)
            dot = glow_dot(fb.get_corner(UR), color=VECTOR_COLOR)
