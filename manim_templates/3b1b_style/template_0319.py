"""
Template 0319 — COMPARE: Start vs End FORCE_COLOR
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            ...
            lt = latin_text("Start", font_size=FONT_SIZE_LABEL, color=FORCE_COLOR)
            lt.move_to(LEFT*3)
            clamp_to_screen(lt)
            rt = latin_text("End", font_size=FONT_SIZE_LABEL, color=SUCCESS_COLOR)
            rt.move_to(RIGHT*3)
            clamp_to_screen(rt)
