"""
Template 0265 — VECTOR: angle_t.get_ STAR_YELLOW
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            ...
            vec = always_redraw(lambda: Arrow(axes.c2p(0,0), axes.c2p(2*np.cos(angle_t.get_value()+PI/2), 2*np.sin(angle_t.get_value()+PI/2)), buff=0, color=STAR_YELLOW, stroke_width=5, max_tip_length_to_length_ratio=0.15))
            self.add(vec)
