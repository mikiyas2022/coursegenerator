"""
Template 0290 — COMPARE: Input vs Output ROSE_COLOR
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            ...
            lt = latin_text("Input", font_size=FONT_SIZE_LABEL, color=ROSE_COLOR)
            lt.move_to(LEFT*3)
            clamp_to_screen(lt)
            rt = latin_text("Output", font_size=FONT_SIZE_LABEL, color=ERROR_COLOR)
            rt.move_to(RIGHT*3)
            clamp_to_screen(rt)
