"""
Template 0305 — COMPARE: Theory vs Practice ROSE_COLOR
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            ...
            lt = latin_text("Theory", font_size=FONT_SIZE_LABEL, color=ROSE_COLOR)
            lt.move_to(LEFT*3)
            clamp_to_screen(lt)
            rt = latin_text("Practice", font_size=FONT_SIZE_LABEL, color=ERROR_COLOR)
            rt.move_to(RIGHT*3)
            clamp_to_screen(rt)
