"""
Template 261 — COMPARE: Hypothesis vs Result
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            ...
            lt = latin_text("Hypothesis", font_size=FONT_SIZE_LABEL, color=SUCCESS_COLOR)
            lt.move_to(LEFT*3)
            clamp_to_screen(lt)
            rt = latin_text("Result", font_size=FONT_SIZE_LABEL, color=TEAL_ACCENT)
            rt.move_to(RIGHT*3)
            clamp_to_screen(rt)
