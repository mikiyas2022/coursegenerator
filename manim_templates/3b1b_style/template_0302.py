"""
Template 0302 — COMPARE: Theory vs Practice STAR_YELLOW
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            ...
            lt = latin_text("Theory", font_size=FONT_SIZE_LABEL, color=STAR_YELLOW)
            lt.move_to(LEFT*3)
            clamp_to_screen(lt)
            rt = latin_text("Practice", font_size=FONT_SIZE_LABEL, color=FORCE_COLOR)
            rt.move_to(RIGHT*3)
            clamp_to_screen(rt)
