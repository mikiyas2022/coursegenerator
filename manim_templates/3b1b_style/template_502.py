"""
Template 502 — DEEP DIVE: y = 0.8sin(2x) ROSE_COLOR
"""
class SceneTemplate(AmharicEduScene):
    def construct(self):
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_0>") as tracker:
            obj = latin_text("<NARRATION_PLACEHOLDER_0>", font_size=FONT_SIZE_HEADER, color=ROSE_COLOR)
            obj.move_to(ORIGIN)
            clamp_to_screen(obj)
        ...
        with self.voiceover(text="<NARRATION_PLACEHOLDER_1>") as tracker:
            ...
            import math
            curve = axes.plot(lambda x: math.sin(x*2)*0.8, x_range=[-3.5,3.5], color=ROSE_COLOR, stroke_width=3)
            lbl = latin_text("y = 0.8sin(2x)", font_size=FONT_SIZE_CAPTION, color=ROSE_COLOR)
